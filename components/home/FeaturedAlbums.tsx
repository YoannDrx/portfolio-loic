...
export default function FeaturedAlbums() {
  const t = useTranslations('home.featuredAlbums');
  const locale = useLocale();
  const albumsData = getAlbumsData(locale);
  // Get the 4 most recent albums
  const featuredAlbums = albumsData.slice(0, 4);
    ...
    <section className="py-24 bg-gradient-to-b from-obsidian to-obsidian-50">
      <div className="container-custom">
        ...
        <AnimatedSection variant="fadeIn" delay={0.5} className="text-center">
          ...
          >
            <span>{t('viewAll', { count: albumsData.length })}</span>
