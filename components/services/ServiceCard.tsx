  ...
  return (
    <Link href={{ pathname: '/services/[id]', params: { id: service.id } }} className="block h-full">
      <GlassCard
        variant="default"
        hover="lift"
        className="group h-full flex flex-col overflow-hidden cursor-pointer border-[var(--glass-border-subtle)] hover:border-neon-cyan/30"
      >
        <GlassCardContent className="p-0 h-full flex flex-col">
          ...
          <div className="relative aspect-video w-full overflow-hidden bg-muted">
            ...
            <div className="absolute top-4 left-4 z-10">
              <div className="px-3 py-1 bg-glass-strong backdrop-blur-md border border-neon-cyan/50 rounded text-neon-cyan font-mono text-xs font-bold tracking-wider shadow-[0_0_10px_rgba(0,240,255,0.3)]">
                NO.{service.no}
              </div>
            </div>
          </div>
          ...
          <div className="p-6 flex-1 flex flex-col bg-gradient-to-b from-[var(--glass-subtle)] to-transparent">
            {/* Title */}
            <h3 className="text-2xl font-bold text-foreground mb-3 group-hover:text-gradient-neon transition-all line-clamp-2 font-montserrat uppercase tracking-wide">
              {service.title}
            </h3>
