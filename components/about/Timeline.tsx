...
export default function Timeline() {
  ...
  return (
    <div className="relative max-w-5xl mx-auto py-4 sm:py-8">
      {/* Vertical Line - closer to edge on mobile */}
      <div className="absolute left-3 sm:left-8 md:left-1/2 top-0 bottom-0 w-0.5 bg-gradient-to-b from-transparent via-white/20 to-transparent transform -translate-x-1/2" />
        ...
        {eventsConfig.map((event, index) => {
          ...
          return (
            ...
            >
              ...
              <div className="absolute left-3 sm:left-8 md:left-1/2 transform -translate-x-1/2 top-4 sm:top-6 z-10">
                <div className={cn(
                  "w-3 h-3 sm:w-4 sm:h-4 rounded-full shadow-[0_0_20px_currentColor] ring-2 sm:ring-4 ring-background",
                  config.dotClass
                )} />
              </div>
              ...
              {/* Content Card */}
              <div className={cn(
                "flex-1 pl-8 sm:pl-20 md:pl-0",
                isLeft ? "md:pr-16" : "md:pl-16"
              )}>
                <div className="relative bg-glass-subtle backdrop-blur-sm border border-[var(--glass-border)] rounded-lg sm:rounded-xl p-4 sm:p-6 hover:border-[var(--glass-border-strong)] hover:bg-glass transition-all duration-300 group">
                  {/* Type Badge + Year - inline on mobile, stacked on larger */}
                  <div className={cn(
                    "flex flex-row sm:flex-col items-center sm:items-start gap-2 sm:gap-3 mb-3 sm:mb-4",
                    isLeft ? "md:items-end" : "items-start"
                  )}>
                    {/* Type Badge */}
                    <div className={cn(
                      "inline-flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-md sm:rounded-lg border",
                      config.badgeBg,
                      config.badgeBorder
                    )}>
                      <Icon className={cn("w-3 h-3 sm:w-3.5 sm:h-3.5", config.badgeText)} />
                      <span className={cn(
                    ...
                    {/* Year */}
                    <div className={cn(
                      "text-xl sm:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r",
                      config.yearGradient
                    )}>
                      {event.year}
                    </div>
                  ...
                  {/* Title */}
                  <h3 className={cn(
                    "text-base sm:text-xl font-bold text-foreground mb-2 sm:mb-3 group-hover:text-gradient-neon transition-all",
